...
If you're using [Microsoft Power Automate flows](https://www.microsoft.com/it-it/power-platform/products/power-automate?market=it) and have a teams webhook, you can use it to retrieve execution data and handle it as needed.
To enable this feature, use the `useTeamsWebhook` option in your reporter configuration:
```
useTeamsWebhook = true
```
Utilizza successivamente `teamsWebhook` per indicare il tuo webhook:
```
teamsWebhook = "https://your-teams-webhook-url-here".
```
The JSON sent via POST contains the following structure:
```json
{
  "jiraIssue": {
    "executionSummary": "",
...
```
The payload adheres to the following schema:
```json
{
  "type": "object",
  "properties": {
